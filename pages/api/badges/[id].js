...
export default function handler(req, res) {
   ...
   switch (reqMethod) {
    ...
    case 'PUT':
        const data = {...req.body};
        const querya ='SELECT * FROM badges WHERE id = ?'
        const queryb= 'UPDATE badges SET ? WHERE id = ?'
        ...
        case 'DELETE':
            const queryx ='SELECT * FROM badges WHERE id = ?'
            const queryy= 'UPDATE badges SET status=0 WHERE id = ?'
